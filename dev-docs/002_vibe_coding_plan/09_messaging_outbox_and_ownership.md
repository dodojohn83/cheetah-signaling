...
- [ ] `CommandBus` 只传递含 operation/step ID 的不可变 CommandEnvelope；ack 不代表 Operation 成功。
...
- [ ] 创建/更新 Operation 的领域事务同时插入 `outbox_events`。
...
- [ ] 重复 Command 返回之前的 step/result 或直接确认，不重复推进 Operation。
- [ ] 去重记录保留期大于消息最大重投窗口。
- [ ] 业务失败与不可反序列化消息使用不同死信原因。

### OWN-001：设备租约

`device_owners` 至少包含 `device_key`、`node_id`、`epoch`、`lease_until`、`updated_at`。获取所有权必须原子增加 epoch；只有持有匹配 epoch 的节点才能更新在线会话、Operation step、MediaSession 或 MediaBinding。
...
- [ ] CommandResult 必须关联 operation/step ID；CAS 推进 Operation 时不得覆盖既有终态。
