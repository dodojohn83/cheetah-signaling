...
- [x] 创建 workspace members，先加入 Phase 0/1 skeleton，后续阶段逐项加入。当前 workspace 包含 apps/cheetah-signaling 与 crates/{foundation,domain,application,runtime,storage,messaging,cluster,media,api,protocols} 及 tests/common。
- [x] 配置 `[workspace.package]`：version `0.1.0`、edition `2024`、双许可证 `MIT OR Apache-2.0`、rust-version `1.96`。
- [x] 配置 workspace rust/clippy lint：`unsafe_code = forbid`、`missing_debug_implementations = warn`、`await_holding_lock = warn`。`missing_docs` 在 skeleton 阶段设为 `allow`。
...
- [x] 创建 `tests/common` 骨架 crate；FakeClock、deterministic IDs、fake secret/media/bus 在后续阶段补充。
- [ ] PostgreSQL/NATS integration 使用可销毁容器或显式环境变量；测试必须有 deadline 和清理。（待 storage/messaging 实现后补充）
- [ ] fixture 目录包含 manifest：来源、协议版本、脱敏、预期结果、许可证。（待协议实现后补充）
- [ ] 测试端口使用 OS 分配，不写死并行冲突端口。（后续测试补充）
- [ ] 所有 ignored interop test 检查 env 后明确 skip；CI 专用 job 必须真正执行而非空通过。（后续 interop 测试补充）
