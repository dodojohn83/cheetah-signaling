...
6. reconciler 处理旧节点的 Operation/MediaBinding。
...
- command envelope 有 deadline；过期 command 进入 TimedOut，不在设备恢复后无限补发。
- gateway/worker inbox 去重后执行，结果发布 OperationStateChanged。
...
Operation/outbox 已持久化，由其他 consumer 接管。Saga step 必须在数据库记录 attempt、idempotency key 和外部 handle，避免重建不同媒体 session。
