...
- [ ] `GB4-MED-001`：把 GbMediaMachine 迁入 core 并接入 transaction/dialog/timer；实现 live start/stop 状态表。
- [ ] `GB4-MED-002`：实现 typed GB command mapper、OperationStep 和 MediaPort Open/Update/Close Saga。
- [ ] `GB4-MED-003`：完成 UDP、TCP active/passive、SSRC/payload/setup 协商和 compatibility policy。
- [ ] `GB4-MED-004`：完成 playback/download/RecordInfo/MANSRTSP/MediaStatus 生命周期。
- [ ] `GB4-MED-005`：完成 talk/Broadcast 与 media sender capability、双侧补偿。
- [ ] `GB4-MED-006`：实现 late/duplicate/early media、CANCEL/BYE、timeout/cancel/UnknownOutcome 状态迁移。
- [ ] `GB4-MED-007`：接入 reconciler，覆盖 signaling/device/media restart 和旧 epoch/instance callback。
- [ ] `GB4-MED-008`：fake 与 real media node 运行同一 GB media contract/system suite。
