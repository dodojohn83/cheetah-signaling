...
- [x] `GB4-SYS-003`：完成真实设备/NVR 互操作报告模板，并补充本地控制面预验证映射；当前 `Blocked`（缺少真实设备/NVR 接入），报告见 [reports/gb4-sys-003.md](reports/gb4-sys-003.md)。
- [x] `GB4-SYS-004`：完成上下级平台级联互操作报告模板，并补充本地控制面预验证映射；当前 `Blocked`（缺少真实上级/下级平台接入），报告见 [reports/gb4-sys-004.md](reports/gb4-sys-004.md)。
