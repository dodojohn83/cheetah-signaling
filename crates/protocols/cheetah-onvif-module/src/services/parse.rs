...
impl<'a> ParseContext<'a> {
    pub(crate) fn new(limits: &'a ParserLimits, input: &str) -> Result<Self, OnvifModuleError> {
    ...
    pub(crate) fn on_start(&mut self, name: String) -> Result<(), OnvifModuleError> {
        self.node_count += 1;
    ...
    pub(crate) fn on_empty(&mut self) -> Result<(), OnvifModuleError> {
        self.node_count += 1;
        if self.node_count > self.limits.max_nodes {
            return Err(limit_error(format!(
                "response exceeds {} nodes",
    ...
    pub(crate) fn append_text(&mut self, s: &str) -> Result<(), OnvifModuleError> {
...
pub(crate) fn limit_error(message: impl Into<String>) -> OnvifModuleError {
