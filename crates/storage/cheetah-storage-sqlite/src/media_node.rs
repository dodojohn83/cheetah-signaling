...
/// SQLite media node repository.
#[derive(Debug, Clone)]
pub struct SqliteMediaNodeRepository {
    read_pool: SqlitePool,
    write_pool: SqlitePool,
}

impl SqliteMediaNodeRepository {
...
impl MediaNodeRepository for SqliteMediaNodeRepository {
    async fn register(
        ...
        match existing {
            None => {
                sqlx::query(&format!(
                    "INSERT INTO media_nodes ({}) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    media_node_columns()
                ))
                .bind(node.node_id.as_uuid())
            ...
            Some(_) => {
                sqlx::query(
                    "UPDATE media_nodes SET
                        instance_id = ?, instance_epoch = ?, zone = ?, region = ?,
                        network_zones = ?, labels = ?, control_endpoint = ?, media_addresses = ?,
                        capabilities = ?, capacity = ?, load = ?, session_count = ?, draining = ?,
                        status = ?, last_heartbeat_at = ?, lease_until = ?, generation = ?,
                        contract_version = ?, revision = ?, updated_at = ?
                     WHERE node_id = ?",
                )
                .bind(&node.instance_id)
                .bind(node.instance_epoch as i64)
        ...
        let row: MediaNodeRow = sqlx::query_as::<sqlx::Sqlite, MediaNodeRow>(&format!(
            "SELECT {} FROM media_nodes WHERE node_id = ?",
            media_node_columns()
        ))
        .bind(node.node_id.as_uuid())
        .fetch_one(&mut *tx)
    ...
    async fn heartbeat(
        ...
        let row: Option<MediaNodeRow> = sqlx::query_as::<sqlx::Sqlite, MediaNodeRow>(&format!(
            "UPDATE media_nodes SET
                    lease_until = ?, last_heartbeat_at = ?, load = ?, session_count = ?,
    ...
    async fn set_draining(
        ...
        let row: Option<MediaNodeRow> = sqlx::query_as::<sqlx::Sqlite, MediaNodeRow>(
            &format!(
                "UPDATE media_nodes SET draining = ?, status = ?, updated_at = ?, revision = revision + 1
                 WHERE node_id = ? AND instance_id = ?
                 RETURNING {}",
                media_node_columns()
            )
        )
        .bind(if draining { 1i32 } else { 0i32 })
        .bind(status)
        .bind(to_millis(updated_at))
        .bind(node_id.as_uuid())
        .bind(&instance_id)
