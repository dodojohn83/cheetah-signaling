//! Protocol session repository for SQLite.
...
/// SQLite protocol session repository.
#[derive(Debug, Clone)]
pub struct SqliteProtocolSessionRepository {
    read_pool: SqlitePool,
    write_pool: SqlitePool,
}

impl SqliteProtocolSessionRepository {
...
impl ProtocolSessionRepository for SqliteProtocolSessionRepository {
    async fn get(
        &self,
        tenant_id: TenantId,
        protocol_session_id: ProtocolSessionId,
    ) -> Result<Option<ProtocolSession>> {
        let row: Option<ProtocolSessionRow> = sqlx::query_as::<sqlx::Sqlite, ProtocolSessionRow>(
            "SELECT protocol_session_id, updated_at, data FROM protocol_sessions
             WHERE tenant_id = ? AND protocol_session_id = ?",
        )
    ...
    async fn get_by_device(
        ...
        let row: Option<ProtocolSessionRow> = sqlx::query_as::<sqlx::Sqlite, ProtocolSessionRow>(
            "SELECT protocol_session_id, updated_at, data FROM protocol_sessions
             WHERE tenant_id = ? AND protocol = ? AND device_id = ?",
        )
        .bind(tenant_id.as_uuid())
    ...
    async fn get_by_identity(
        ...
        let row: Option<ProtocolSessionRow> = sqlx::query_as::<sqlx::Sqlite, ProtocolSessionRow>(
            "SELECT protocol_session_id, updated_at, data FROM protocol_sessions
             WHERE tenant_id = ? AND protocol = ? AND protocol_identity = ?",
        )
        .bind(tenant_id.as_uuid())
        .bind(protocol.to_string())
    ...
    async fn save(&mut self, session: &ProtocolSession) -> Result<()> {
        ...
        if result.rows_affected() != 1 {
            let found: Option<(i64,)> = sqlx::query_as(
                "SELECT revision FROM protocol_sessions WHERE protocol_session_id = ?",
            )
            .bind(session.protocol_session_id().as_uuid())
    ...
    async fn delete(
        ...
        let result = sqlx::query(
            "DELETE FROM protocol_sessions
             WHERE tenant_id = ? AND protocol_session_id = ? AND revision = ?",
        )
        .bind(tenant_id.as_uuid())
        .bind(protocol_session_id.as_uuid())
        ...
        let found: Option<(i64,)> = sqlx::query_as(
            "SELECT revision FROM protocol_sessions WHERE tenant_id = ? AND protocol_session_id = ?",
        )
        .bind(tenant_id.as_uuid())
        .bind(protocol_session_id.as_uuid())
        .fetch_optional(&self.write_pool)
