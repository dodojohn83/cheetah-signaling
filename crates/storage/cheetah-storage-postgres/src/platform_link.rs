//! Cascade platform link repository for PostgreSQL.
...
/// PostgreSQL cascade platform link repository.
#[derive(Debug, Clone)]
pub struct PostgresPlatformLinkRepository {
    read_pool: PgPool,
    write_pool: PgPool,
}

impl PostgresPlatformLinkRepository {
...
impl PlatformLinkRepository for PostgresPlatformLinkRepository {
    async fn get(
        &self,
        tenant_id: TenantId,
        platform_link_id: PlatformLinkId,
    ) -> Result<Option<GbPlatformLink>> {
        let row: Option<PlatformLinkRow> = sqlx::query_as::<sqlx::Postgres, PlatformLinkRow>(
            "SELECT platform_link_id, updated_at, data FROM platform_links
             WHERE tenant_id = $1 AND platform_link_id = $2",
        )
    ...
    async fn get_by_remote_identity(
        ...
        let row: Option<PlatformLinkRow> = sqlx::query_as::<sqlx::Postgres, PlatformLinkRow>(
            "SELECT platform_link_id, updated_at, data FROM platform_links
             WHERE tenant_id = $1 AND direction = $2 AND remote_identity = $3",
        )
        .bind(tenant_id.as_uuid())
    ...
    async fn save(&mut self, link: &GbPlatformLink) -> Result<()> {
        ...
        if result.rows_affected() != 1 {
            let found: Option<(i64,)> =
                sqlx::query_as("SELECT revision FROM platform_links WHERE platform_link_id = $1")
                    .bind(link.platform_link_id().as_uuid())
    ...
    async fn delete(
        ...
        let result = sqlx::query(
            "DELETE FROM platform_links
             WHERE tenant_id = $1 AND platform_link_id = $2 AND revision = $3",
        )
        .bind(tenant_id.as_uuid())
        .bind(platform_link_id.as_uuid())
        ...
        let found: Option<(i64,)> = sqlx::query_as(
            "SELECT revision FROM platform_links WHERE tenant_id = $1 AND platform_link_id = $2",
        )
        .bind(tenant_id.as_uuid())
        .bind(platform_link_id.as_uuid())
        .fetch_optional(&self.write_pool)
