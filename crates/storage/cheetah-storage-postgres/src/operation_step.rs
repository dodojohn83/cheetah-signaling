//! Operation step repository for PostgreSQL.
...
/// PostgreSQL operation step repository.
#[derive(Debug, Clone)]
pub struct PostgresOperationStepRepository {
    pool: PgPool,
}

impl PostgresOperationStepRepository {
...
impl OperationStepRepository for PostgresOperationStepRepository {
    ...
    async fn list(
        ...
        let rows: Vec<OperationStepRow> = sqlx::query_as::<sqlx::Postgres, OperationStepRow>(
            "SELECT data FROM operation_steps WHERE tenant_id = $1 AND operation_id = $2 ORDER BY attempt",
        )
