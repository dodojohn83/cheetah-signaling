...
impl MediaClusterRegistry for MediaClusterRegistryService {
    ...
    async fn heartbeat_media_node(
        ...
        validate_node_id(&heartbeat.node_id, &self.config)?;
    ...
    async fn drain_media_node(
        ...
        validate_node_id(&drain.node_id, &self.config)?;
    ...
    async fn deregister_media_node(
        ...
        validate_node_id(&deregister.node_id, &self.config)?;
...
fn validate_node_id(node_id: &str, config: &MediaRegistryConfig) -> Result<(), Status> {
    if node_id.len() > config.max_string_field_length {
        return Err(Status::invalid_argument(format!(
            "node_id exceeds {} bytes",
            config.max_string_field_length
        )));
    }
    Ok(())
}
...
fn validate_registration_fields(
    ...
    if registration.network_zones.len() > config.max_network_zones {
        return Err(Status::invalid_argument(format!(
            "network_zones exceeds {} entries",
            config.max_network_zones
        )));
    }
    ...
    #[allow(deprecated)]
    {
        let capability_count = registration.capabilities.len()
            + if registration.capability.is_some() {
                1
            } else {
                0
            };
        if capability_count > config.max_capabilities {
            return Err(Status::invalid_argument(format!(
